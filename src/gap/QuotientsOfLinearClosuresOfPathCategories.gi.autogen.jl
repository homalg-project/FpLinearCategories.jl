...
@BindGlobal( "QUOTIENT_CATEGORY_OF_LINEAR_CLOSURE_OF_PATH_CATEGORY",
  ...
  function ( CAP_NAMED_ARGUMENTS, kC, relations )
    ...
    if (HasFiniteNumberOfMacaulayMorphisms( C, leading_monomials ))
        ...
        range_HomStructure = range_of_HomStructure;
        
        if (range_HomStructure == fail)
            
            if (HasRangeCategoryOfHomomorphismStructure( kC ))
        ...
        AddCoefficientsOfMorphism( quo_kC,
          function ( quo_kC, mor )
            local kC, s, t, support_mor, coeffs;
            ...
            return List( ExternalHoms( quo_kC )[s][t], coeffs );
...
@BindGlobal( "QUOTIENT_CATEGORY_OF_LINEAR_CLOSURE_OF_QUOTIENT_OF_PATH_CATEGORY",
  ...
  function ( k_quo_C, relations )
    ...
    if (@not IsQuotientOfPathCategory( quo_C ))
    ...
    ##
    name = List( relations[ (1):(Minimum( 3, Length( relations ) )) ], str_func );
    ...
    object_constructor =
      function ( quo_k_quo_C, obj )
        ...
        return CreateCapCategoryObjectWithAttributes( quo_k_quo_C,
                       UnderlyingCell, obj );
    ...
    morphism_constructor =
      function ( quo_k_quo_C, S, mor, T )
        
        return CreateCapCategoryMorphismWithAttributes( quo_k_quo_C,
                       S,
                       T,
                       UnderlyingCell, mor );
    ...
    rel_2 = List( relations, m ->
                   CallFuncListAtRuntime(
                           ...
                           [ kC,
                             ...
                             PairGAP( CoefficientsList( m ), List( SupportMorphisms( m ), CanonicalRepresentative ) ),
    ...
    modeling_tower_morphism_constructor =
      function ( quo_k_quo_C, source, mor_datum, target )
        ...
        return MorphismConstructor( quo_kC,
                       source,
                       MorphismConstructor( kC, ObjectDatum( quo_kC, source ), PairGAP( coeffs, suppor ), ObjectDatum( quo_kC, target ) ),
                       target );
    ...
    if (HasIsEquippedWithHomomorphismStructure( quo_kC ) && IsEquippedWithHomomorphismStructure( quo_kC ))
...
@InstallMethod( QuotientCategory,
  ...
  function( kC, relations )
    ...
    elseif (IsQuotientOfPathCategory( C ))
...
@InstallGlobalFunction( INSTALL_CANONICAL_REPRESENTATIVE_METHODS_IN_QUOTIENT_CATEGORIES_OF_LINEAR_CLOSURES_OF_PATH_CATEGORIES_OR_THEIR_QUOTIENTS,
  ...
  function ( quo_cat )
    ...
    if (IsPathCategory( UnderlyingCategory( AmbientCategory( quo_cat ) ) ))
