...
@BindGlobal( "CREATE_NAME_FOR_ALGEBROID_FROM_DATA_TABLES",
  ...
  function ( coefficients_ring, q )
    ...
    return @Concatenation(
                ...
                gm_string,
                "] ) defined by ",
                StringGAP( NumberOfObjects( q ) ),
                " object",
                with_or_without_s( NumberOfObjects( q ) ),
                " and ",
                StringGAP( NumberOfMorphisms( q ) ),
                " generating morphism",
...
@InstallMethod( AlgebroidFromDataTables,
  ...
  function ( CAP_NAMED_ARGUMENTS, input_data )
    ...
    range_category_of_hom_structure = range_of_HomStructure;
    
    if (range_category_of_hom_structure == fail)
    ...
    AddHomomorphismStructureOnMorphismsWithGivenObjects( A,
      ...
      function ( A, s, alpha, gamma, r )
        ...
        if ((RankOfObject( s ) == 0 || Length( IndicesOfSupportMorphisms( alpha ) ) == 0)  || (RankOfObject( r ) == 0 || Length( IndicesOfSupportMorphisms( gamma ) ) == 0))
...
@InstallMethod( /,
  ...
  function ( name, A )
    ...
    if (ForAny( [ "⋅", "*" ], s -> PositionSublist( name, s ) != fail ))
      ...
      p = PositionProperty( mors_labels, label -> Length( name ) >= Length( label ) && IsMatchingSublist( name, label, Length( name ) - Length( label ) + 1 ) );
...
@InstallMethod( DecompositionOfMorphismInAlgebroid,
  ...
  function ( mor )
    ...
    return List( DecompositionIndicesOfMorphismInAlgebroid( mor ),
                    function ( p )
                      if (IsEmpty( p[2] ))
                        return PairGAP( p[1], [ IdentityMorphism( A, Source( mor ) ) ] );
                      else
                        return PairGAP( p[1], SetOfGeneratingMorphisms( A )[ p[2] ] );
                      end;
                    end );
...
@InstallMethod( IsAdmissibleAlgebroid,
  ...
  function ( A )
    ...
    if (dim[i] != NumberOfObjects( q ) + NumberOfMorphisms( q ))
        return false;
    end;
...
InstallMethodWithCache( TensorProductOfAlgebroids,
  ...
  function ( A_1, A_2 )
    ...
    if (@not IsIdenticalObj( CommutativeRingOfLinearCategory( A_1 ), CommutativeRingOfLinearCategory( A_2 ) ))
...
@InstallMethod( ViewString,
  ...
  function ( alpha )
    ...
    if (IsEmpty( indices_nonzero_coeffs ))
        datum_string = @Concatenation( colors.coeff, "0", colors.reset );
    else
        coeffs = List( coeffs[indices_nonzero_coeffs], StringGAP );
